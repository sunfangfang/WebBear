...
function calendar(y, m) {
    ...
    if (m == 3) this[tmp1].color = '#ea544a'; //清明颜色
    ...
    for (i  in  sFtv)
        if (sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
            if (Number(RegExp.$1) == (m + 1)) {
                ...
                if (RegExp.$3 == '*')  this[Number(RegExp.$2) - 1].color = '#ea544a'
            }
        ...
        if (lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
            ...
            if (tmp1 >= 0 && tmp1 < n) {
                ...
                if (tmp2 >= 0 && tmp2 < this.length) {
                    ...
                    if (RegExp.$3 == '*')  this[tmp2].color = '#ea544a'
                }
            }
        }
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            //wly 注册点击事件
						...
						if(hstr.indexOf(nowDays)>-1){
							 $("#GD" + i).addClass("selday").siblings().removeClass(selday);
							}
            ...
            if (s.length > 0) {  //农历节日
                ...
                s = s.fontcolor('#f8a42a');
            }
            else {  //国历节日
                s = cld[sD].solarFestival;
                if (s.length > 0) {
                    if (s.length > 8)  s = s.substr(0, 5) + '...';
                    s = (s == '黑色星期五') ? s.fontcolor('f8a42a') : s.fontcolor('#f8a42a');
                }
                else {  //廿四节气
                    s = cld[sD].solarTerms;
                    if (s.length > 0)  s = s.fontcolor('#f8a42a');
                }
            }
            if (cld[sD].solarTerms == '清明') s = '清明节'.fontcolor('#ea544a');
            if (cld[sD].solarTerms == '芒种') s = '芒种'.fontcolor('#ea544a');
            if (cld[sD].solarTerms == '夏至') s = '夏至'.fontcolor('#ea544a');
            if (cld[sD].solarTerms == '冬至') s = '冬至'.fontcolor('#ea544a');
...
function mOvr(thisObj, v) {
    ...
    if (sObj.html() != '') {
        ...
        arr.push('<font color="#f8a42a" style="font-weight:bold;font-size:13px;">    ' + cld[d].sYear + '年' + cld[d].sMonth + '月' + cld[d].sDay + '日</font>');
...
function mOck(thisObj, v){
	...
	if(onoff == '0'){
		...
		thisObj.setAttribute("id", "selday"); 
		thisObj.attributes["on"].value='1';
		hDays.push(dayJson);
   }else{
   	//thisObj.style.background='';
   	thisObj.setAttribute("id", "");
